...
function buildFakeDialogShell(root, markup) {
  ...
  mask.className = 'task-dialog-mask shared-overlay-mask';
...
globalThis.location = {
  ...
  hostname: 'localhost',
  pathname: '/login.html',
  href: 'http://localhost:8080/login.html'
};
...
assert(grouped.q1.length === 0, 'Q1 完成任务应被过滤');
...
assert(finalList.data.length === 2, '最终任务数量不正确');
assert(finalList.data.some(t => t._id === task2.data._id), '最终保留任务不正确');
